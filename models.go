...
func listIntervals(db *sql.DB, userID int64) ([]Interval, error) {
	rows, err := db.Query(
		`SELECT i.id, i.name, i.start_date, i.end_date, i.color, sg.id, sg.name, sg.public_slug
		FROM intervals i
...
func createInterval(db *sql.DB, userID int64, iv Interval) (Interval, error) {
	...
	res, err := db.Exec(
		`INSERT INTO intervals (user_id, name, start_date, end_date, color, share_group_id, visibility) VALUES (?, ?, ?, ?, ?, ?, 'private')`,
		userID, iv.Name, iv.StartDate, iv.EndDate, iv.Color, iv.ShareGroupID,
	)
	if err != nil {
		return Interval{}, err
	}
...
func intervalByID(db *sql.DB, userID, id int64) (Interval, error) {
	...
	err := row.Scan(&iv.ID, &iv.Name, &iv.StartDate, &iv.EndDate, &iv.Color, &shareGroupID, &shareGroupName, &shareGroupSlug)
	if err != nil {
...
func scanIntervalRow(scanner interface {
	...
	var shareGroupName sql.NullString
	var shareGroupSlug sql.NullString
	if err := scanner.Scan(&iv.ID, &iv.Name, &iv.StartDate, &iv.EndDate, &iv.Color, &shareGroupID, &shareGroupName, &shareGroupSlug); err != nil {
		return Interval{}, err
	}
	if shareGroupID.Valid {
		id := shareGroupID.Int64
