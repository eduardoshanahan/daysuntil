(() => {
  ...
  const daysUntilApiClient = {
    ...
    listReminders: intervalId => apiFetch(`/api/intervals/${intervalId}/reminders`),
    createReminder: (intervalId, data) => apiFetch(`/api/intervals/${intervalId}/reminders`, { method: 'POST', body: JSON.stringify(data) }),
    updateReminder: (id, data) => apiFetch(`/api/reminders/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
    deleteReminder: id => apiFetch(`/api/reminders/${id}`, { method: 'DELETE' }),
